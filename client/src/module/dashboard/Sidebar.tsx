...
import { handleCallLogout } from "../../redux/apiRequest";
  ...
  const handleLogout = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Logged out!", "Login success fully", "success");
      }
    });
  };
  return (
    <>
      ...
      <aside
        ...
        className="pt-[100px] fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0"
        aria-label="Sidebar"
      >
        <div className="h-full px-3 py-16 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          {currentUser?.userInfo?.role === 1 ? (
            <ul className="space-y-2 font-medium">
              {sidebarPrivate.map((link) => {
                return (
                  <li key={link.title}>
                    <Link
                      to={link.url}
